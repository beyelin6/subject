...
export default function App() {
  ...
  const smartJoin = (arr: string[]): string => {
    ...
    if (arr.length === 2) return `${arr[0]}，且${arr[1]}`;
    return arr.slice(0, -1).join('、') + '，並' + arr[arr.length - 1];
  };
  ...
  const generatedCommentsMap = useMemo(() => {
    ...
    return result;
  }, [compiledGroups, studentName, pronoun, currentGrade, currentSubject, activeDataset, subjectLabel]);
