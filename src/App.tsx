...
export default function App() {
  ...
  const compileAdvancedRhetoricComment = (
    ...
    if (key === 'struggling') {
      return `在${sub}學習上遇到了一些小瓶頸，平常對於${smartJoin(neg) || '基本概念的理解'}感到有些吃力。但孩子其實想做好，只要我們多加陪伴、耐心地帶他多做基礎練習，相信能漸入佳境！`;
    }
    if (key === 'slowButGood') {
      return `做事雖然慢一點，但腳踏實地。課堂上還是能認真做好${smartJoin(allParts)}。在學習上遇到${smartJoin(neg) || '吸收較慢的困難'}時不要急著氣餒，多給他一點消化時間，他一定能學得很好！`;
    }
    if (key === 'activeDistracted') {
      return `活潑好動、很有朝氣，在${sub}練習時能做到${smartJoin(allParts)}。就是上課定性還不太夠，容易因為太興奮而造成${smartJoin(neg) || '容易分心'}，如果能多練習讓自己專心，成果會更出色！`;
    }
    if (key === 'cooperative') {
      return `非常熱心、有團隊合作精神，在組內很會幫忙別人並做到${smartJoin(allParts)}。如果能多把注意力分給自己，改善${smartJoin(neg) || '作業或課後複習'}，成績表現會更均衡！`;
    }
    if (key === 'impatient') {
      return `上課算積極，也常能配合做好${smartJoin(allParts)}。不過耐心 and 耐挫力稍微不夠，遇到比較複雜的題目容易${smartJoin(neg) || '想要逃避'}，只要多堅持、有耐性一點，實力絕對沒問題！`;
    }

    // Style Keys templates
    if (key === 'inspiring') {
      return `很有學習熱情，平時能把${smartJoin(allParts)}做得很好。雖然偶爾在${smartJoin(neg) || '一些小地方遇到挑戰'}，這都是必經的練習過程，老師相信你一定能越做越棒！`;
    }
    if (key === 'gentle') {
      return `你是個溫柔、懂事又貼心的孩子。平常功課能做好${smartJoin(allParts)}，真的很棒！如果遇上${smartJoin(neg) || '不會寫的作業'}也別氣餒，放鬆心情跟著老師慢慢學，我們一起克服。`;
    }
    if (key === 'growth') {
      return `這學期在${smartJoin(allParts)}上表現得很不錯。面對${smartJoin(neg) || '一些寫不好的地方'}也別擔心，這些正是我們下學期的學習新目標，跨過去就是很大的進步喔！`;
    }
    if (key === 'academic') {
      return `這學期學習情況良好。平常在${smartJoin(allParts)}很穩定，就是偶爾在${smartJoin(neg) || '寫作或訂正習慣上'}稍微粗心，期待養成更好的複習習慣，讓基礎更扎實。`;
    }
    if (key === 'potential') {
      return `頭腦非常靈活、有滿滿的潛力。平時上課能好好的完成${smartJoin(allParts)}，就是偶爾有點${smartJoin(neg) || '不夠專心'}。如果能收心、定下心來學習，表現一定更驚人！`;
    }
    if (key === 'collaboration') {
      return `這學期在學校能順利展現${smartJoin(allParts)}，表現值得讚賞。不過平時在${smartJoin(neg) || '寫作業或是訂正時'}稍微粗心，還需要家長 and 老師一邊陪伴引導，協助他養成好習慣。`;
    }
    if (key === 'advice') {
      return `底子很好，平常能熟練做好${smartJoin(allParts)}。如果想減少${smartJoin(neg) || '考試或作答粗心'}的失誤，建議多去整理錯題、多看基礎觀念，一定會進步得更快。`;
    }
    if (key === 'steadfast') {
      return `步伐很沉穩踏實！平時上課不只把${smartJoin(allParts)}做好，遇到${smartJoin(neg) || '不會寫的地方'}也默默努力、不急不躁，這種踏實進取的性情非常難得！`;
    }
    if (key === 'energetic') {
      return `在班上是大家的活力之源！課堂中能神采奕奕地做到${smartJoin(allParts)}。如果能多練習讓自己定下心、避免那些因${smartJoin(neg) || '浮躁導致的小失誤'}，表現會更卓越！`;
    }
    if (key === 'classic') {
      return `這學期在課堂上表現穩定，能做好${smartJoin(allParts)}。偶爾在${smartJoin(neg) || '一些小細節上'}多用心寫、避免粗心，未來一定會學得更好。`;
    }
